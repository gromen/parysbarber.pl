...
// Standalone port of src/lib/push.ts's sendPushNotification for this worker.
// Kept as a self-contained copy (rather than an import) because the Astro app's
// version imports Astro's own `Env` type and lives in a different bundle/project —
// see zakres-prac.md section 3 and the task brief for why this worker is separate.
...
export async function sendPushNotification(
  ...
  try {
    ...
    // object) and `undefined` values (from omitted `url`/`tag`) aren't valid JSON —
    // rebuild as a clean plain object, dropping undefined keys.
